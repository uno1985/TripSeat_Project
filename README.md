...
## 3️⃣ 上傳個人分支前請先對齊dev （GitHub PR：feature → dev）
**在確認要上傳自己的分支前，請先對準最新dev**
...
1. 此步驟為查看遠端最新狀態不會更改本機檔案
2. merge 最新的 `dev` 分支 到自己的檔案 **此時若有衝突需要自行解決**


### 解決衝突完畢或順利合併後，PUSH個人分支到遠端
  
```bash
git push origin feature/uno
```

> ❗ PUSH 完成請自行透過線上PR merge 進 `dev` 完成更新

## 4️⃣ 每次開發刪除個人開發分支並重新建立(選用)
...
3. 刪除本機個人支線
4. 建立新的個人支線
5. 推到 GitHub，告知你有一條全新分支
  

> ### 注意事項：
> 若選擇每週刪除本機分支重新建立，請依以上步驟執行，
> 並且需要手動刪除GitHub上自己的feature分支
>```bash
