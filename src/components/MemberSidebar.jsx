  ...
  useEffect(() => {
    ...
    const fetchUnreadCount = async () => {
      ...
      const notifUnread = (notifRes.data || []).filter(
        (m) => !m.deleted_at && !m.is_read && m.sender_id
      ).length;
    ...
    <div className="member-sidebar-container">
      ...
      <div className="d-block d-md-none mb-4">
        <div className="dropdown w-100">
          ...
          <ul
            className={`dropdown-menu w-100 shadow-sm border-0 mt-1 ${isOpen ? 'show' : ''}`}
          ...
          <ul className="list-unstyled mb-0">
            {menuItems.map((item) => (
              <li key={item.path} className="mb-3">
                <Link
                  to={item.path}
                  className={`nav-link py-2 fs-5 fw-medium transition-all d-flex align-items-center justify-content-center gap-2 ${
                    location.pathname === item.path
                      ? 'trip-text-primary-1000 trip-bg-primary-200'
                      : 'text-dark'
                  }`}
