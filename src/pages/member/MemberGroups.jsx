...
const MemberGroups = () => {
      ...
      (participantsRes.data || [])
        ...
        .forEach((p) => {
          ...
          const item = {
            ...
            avatar:
              profile.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${p.user_id}`,
  ...
  const otherTrips = useMemo(() => trips.filter((t) => t.id !== openTrip?.id), [trips, openTrip]);
  const pendingApplicants = useMemo(() => {
    if (!openTrip) return [];
    return (applicantsByTrip[openTrip.id] || []).filter(
      (a) => !approvedIds.includes(a.participantId)
    );
  ...
  const handleApproveApplicant = async (applicant) => {
    ...
    try {
        ...
        axios.patch(
          `${API_URL}/664/trips/${openTrip.id}`,
          {
            current_participants: Math.max(
              (openTrip.current_participants || 1) + (applicant.joinCount || 1),
              0
            ),
            updated_at: now,
          },
      ...
      setApplicantsByTrip((prev) => ({
        ...prev,
        [openTrip.id]: (prev[openTrip.id] || []).filter(
          (a) => a.participantId !== applicant.participantId
        ),
      }));
      ...
      setTrips((prev) =>
        prev.map((trip) => {
          ...
          const nextTrip = {
            ...trip,
            current_participants: Math.max(
              (trip.current_participants || 1) + (applicant.joinCount || 1),
              0
            ),
          };
  ...
  const handleRejectApplicant = async (applicant) => {
    if (
      ...
      processingIds.includes(applicant.participantId)
    )
      return;
    const token = getToken();
    if (!token) {
    ...
    <div className="my-groups-page">
      ...
      <div className="my-groups-header mb-4">
        <div className="d-flex justify-content-between align-items-center flex-wrap gap-3">
          <div>
            ...
            <p className="trip-text-m trip-text-gray-400 mt-1 mb-0">管理你建立的所有旅程揪團</p>
          </div>
      ...
      <div className="row g-3 mb-4">
        ...
        <div className="col-6 col-md-3 my-groups-stat-col">
          <div className="my-groups-stat-card">
            <div
              className="my-groups-stat-number"
      ...
      {/* --- 卡片 1：招募中 (有待審核) --- */}
      {openTrip && (activeFilter === 'all' || activeFilter === 'open') && (
            ...
            <div className="col-md-3">
              <div className="my-groups-card-img-wrapper">
                <img
                  src={
                    openTrip.image_url ||
                    'https://images.unsplash.com/photo-1501555088652-021faa106b9b?w=400&h=300&fit=crop&q=80'
                  }
                  alt={openTrip.title}
            ...
            <div className="col-md-9">
              <div className="my-groups-card-body">
                ...
                <div className="d-flex justify-content-between align-items-start mb-2">
                  <div>
                    <h5 className="my-groups-card-title">{openTrip.title}</h5>
                    <div className="my-groups-card-tags">
                      {(openTrip.tags || []).slice(0, 3).map((tag) => (
                        <span key={tag} className="my-groups-tag">
                          {tag}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="my-groups-card-actions">
                    <Link
                      to={`/member/create-group?tripId=${openTrip.id}`}
                      className="btn btn-sm my-groups-btn-edit"
                      title="編輯"
                    >
                ...
                <div className="my-groups-card-info">
                  <span>
                    ...
                    {formatDateRange(openTrip.start_date, openTrip.end_date)}
                  </span>
                  <span>
                  ...
                  <span>
                    ...
                    {openTrip.current_participants || 0} / {openTrip.max_people || 0} 人
                  </span>
                  <span>
                    <i className="bi bi-clock me-1"></i>剩餘 {daysUntil(openTrip.deadline) ?? '--'}{' '}
                    天截止
                  </span>
                </div>
                ...
                <div className="my-groups-pending-alert">
                  <i className="bi bi-bell-fill me-2"></i>有{' '}
                  <strong>{pendingApplicants.length}</strong> 位旅伴等待審核
                  <button
                    className="btn btn-sm my-groups-btn-expand ms-auto"
                    type="button"
                    onClick={() => setExpandedPending((prev) => !prev)}
                  >
                    <i
                      className={`bi ${expandedPending ? 'bi-chevron-up' : 'bi-chevron-down'}`}
                    ></i>
                    ...
                    {pendingApplicants.map((applicant) => (
                      <div key={applicant.participantId} className="my-groups-applicant">
                        ...
                        <div className="my-groups-applicant-info">
                          <span className="my-groups-applicant-name">
                            {applicant.name} (共{applicant.joinCount}人)
                          </span>
                ...
                <div className="my-groups-members-section">
                  <button
                    ...
                    type="button"
                    onClick={() => setExpandedMembers((prev) => !prev)}
                  >
                    <i className="bi bi-people-fill me-1"></i>
                    查看目前團員 ({members.length - pendingApplicants.length})
                    <i
                      className={`bi ms-1 ${expandedMembers ? 'bi-chevron-up' : 'bi-chevron-down'}`}
                    ></i>
                  </button>
                        ...
                        .map((member) => (
                          <div key={member.participantId} className="my-groups-member">
                            <img
                              src={member.avatar}
                              alt={`${member.name} 頭像`}
                              className="my-groups-member-avatar"
                            />
                            ...
                            {member.role === 'member' && (
                              <span className="my-groups-member-badge">{member.phone}</span>
                            )}
                ...
                {actionError && (
                  <div className="alert alert-warning py-2 mt-3 mb-0">{actionError}</div>
                )}
                ...
                <div className="row g-0">
                  <div className="col-md-3">
                    <div className="my-groups-card-img-wrapper">
                      ...
                      />
                      <span
                        className={`my-groups-status-badge ${trip.statusType === 'draft' ? 'my-groups-status-open' : `my-groups-status-${trip.statusType}`}`}
                      >
                        {trip.statusText}
                      </span>
                    </div>
                  ...
                  <div className="col-md-9">
                    <div className="my-groups-card-body">
                      ...
                      <div className="my-groups-card-info">
                        ...
                        <span>
                          <i className="bi bi-people me-1"></i>
                          {trip.current_participants || 0} / {trip.max_people || 0} 人
                        </span>
                      </div>
                      ...
                      <div className="my-groups-members-section">
                        ...
                        >
                          ...
                          <i
                            className={`bi ms-1 ${isExpanded ? 'bi-chevron-up' : 'bi-chevron-down'}`}
                          ></i>
                        </button>
                      ...
                      <div className="my-groups-card-footer">
                        ...
                        <div className="my-groups-card-footer-right">
                          <Link
                            to={`/member/create-group?tripId=${trip.id}`}
                            className="btn btn-sm my-groups-btn-edit"
                            title="編輯"
                          >
                            <i className="bi bi-pencil-square"></i>
