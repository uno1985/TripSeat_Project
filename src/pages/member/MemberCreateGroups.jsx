...
const MemberCreateGroups = () => {
  ...
  const addItineraryItem = (dayId) => {
    setItineraryDays((prev) =>
      prev.map((day) =>
        day.id === dayId ? { ...day, items: [...day.items, createItineraryItem()] } : day
      )
    );
  };
  ...
  const saveTrip = async ({ status, requireComplete, redirectToTrip }) => {
    ...
    try {
      ...
      const itineraryPayloads = itineraryDays.flatMap((day) =>
        day.items
      ...
      const uniqueItineraryPayloads = itineraryPayloads.filter((item, index, arr) => {
        ...
        return (
          ...
          arr.findIndex(
            (candidate) =>
              `${candidate.day}__${candidate.time}__${candidate.icon}__${candidate.title}__${candidate.note}` ===
              key
          )
        );
      });
      ...
      if (editTripId) {
        const existingItinerariesRes = await axios.get(
          `${API_URL}/664/itineraries?trip_id=${editTripId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  const handlePublish = () =>
    saveTrip({ status: 'open', requireComplete: true, redirectToTrip: true });
  const handleSaveDraft = () =>
    saveTrip({ status: 'draft', requireComplete: false, redirectToTrip: false });
  ...
  return (
    <div className="create-group-page">
      ...
      <div className="create-group-card mb-4">
        ...
        <div className="row g-3 mb-3">
          <div className="col-md-6">
            ...
            <select
              className="form-select create-group-input"
              id="tripCategory"
      ...
      <div className="create-group-card mb-4">
        ...
        {/* 其他圖片 */}
        <div>
          <label className="form-label create-group-label">其他照片（最多 4 張）</label>
          <div className="row g-3">
            {[1, 2, 3, 4].map((n) => (
              <div className="col-md-4" key={n}>
                ...
                >
                  {relatedPreviewUrls[n - 1] ? (
                    ...
                    <div className="create-group-upload-placeholder">
                      <i
                        className={`bi ${relatedUploading[n - 1] ? 'bi-arrow-repeat' : 'bi-plus-lg'}`}
                      ></i>
      ...
      <div className="create-group-card mb-4">
                ...
                <div className="row g-2 align-items-end">
                  ...
                  <div className="col-md-3">
                    ...
                    <input
                      ...
                      value={item.title}
                      onChange={(e) =>
                        updateItineraryItem(day.id, item.id, 'title', e.target.value)
                      }
                    />
                  </div>
                  <div className="col-md-4">
        ...
        <button
          type="button"
          className="btn create-group-btn-add-day mt-3"
